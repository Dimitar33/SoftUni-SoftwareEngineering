...
function registerView(name, section, setup, linkId) {
    ...
    view[name] = setup(section, navigation);
...
function setNav() {
    ...
    if (sessionStorage.getItem(`token`)) {
        ...
        nav.querySelector(`#logoutLink`).style.display = `block`; // may be inline-block!!
        nav.querySelector(`#createLink`).style.display = `block`;
...
nav.addEventListener(`clicl`, async (ev) => {
    ev.preventDefault();
